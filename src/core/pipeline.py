...
def _choose_final_data(state: PipelineState) -> Tuple[str, str, str]:
    ...
    if "json_extract_metadata" in state.data:
        meta = state.data["json_extract_metadata"].get("metadata", {})
        title = meta.get("title")
        author = meta.get("creator")
        if title and author:
            # Simple nettoyage
            if isinstance(author, list): author = ', '.join(author)
            logger.debug(f"Choix final pour {state.file_path.name}: EPUB metadata")
            return title, author, "metadata"
            
    # TODO: Ajouter d'autres logiques (Flowise, etc.)
    ...
    try:
        ...
        if not dry_run:
            ...
            if existing_book:
                ...
                logger.info(f"[{state.final_status.upper()}] {file_path.name}")
                return # Arrêt anticipé

        # 3. Création de l'entrée en DB
        if not dry_run:
            db_data = {
                "file_hash": state.data["file_hash"],
                "filename": file_path.name,
                "file_path": file_path,
                "file_size": file_path.stat().st_size,
            }
            state.book_id = await db.create_book_entry(pool, db_data)
        ...
        state.data["json_extract_text"] = extract.extract_text_preview(file_path, settings.text_preview_chars)
        ...
        if isbn_info.get("isbn"):
            ...
            if not dry_run:
                ...
                if existing_isbn:
                    ...
                    logger.info(f"[{state.final_status.upper()}] {file_path.name} (ISBN: {isbn_info['isbn']})")
        ...
        if dry_run:
            print(f"[DRY-RUN] {file_path.name} -> Status: {state.final_status}")
            # print(json.dumps(state.data, indent=2, default=str)) # Pour debug
        elif state.book_id:
            await db.update_book_entry(pool, state.book_id, state.data)
        
        status_map = {"processed": "OK", "failed": "FAILED"}
        log_status = status_map.get(state.final_status, state.final_status.upper())
        
        if state.final_status == 'processed':
            logger.info(f"[{log_status}] {file_path.name} -> \"{state.data['final_title']}\" / \"{state.data['final_author']}\" (source={state.data['choice_source']}, {state.data['processing_time_ms']}ms)")
        elif state.final_status != 'duplicate_hash':
            logger.info(f"[{log_status}] {file_path.name} : {state.error_message or 'Raison inconnue'}")
